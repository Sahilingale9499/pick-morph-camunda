...
public class TransactionUpdateListener {
     ...
     */
    @KafkaListener(
            topics = "${kafka.topics.transaction-updates}",
            groupId = "transaction-consumer-group",
            containerFactory = "jsonKafkaListenerContainerFactory"
    )
    public void listen(@Payload String payload) {
        KafkaEventEnvelope<TransactionUpdate> event;
        try {
            event = objectMapper.readValue(payload,
                    new TypeReference<KafkaEventEnvelope<TransactionUpdate>>(){});
        } catch (Exception e) {
            log.error("Failed to deserialize transaction-updates-topic envelope: {}", e.getMessage());
            return;
        }
        TransactionUpdate transactionUpdate = event.getPayload();
