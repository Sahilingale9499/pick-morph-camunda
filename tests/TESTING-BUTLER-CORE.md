...
**What happens:** AE publishes `pick_transaction`. Both containers picked — `3000000001` and `3000000002`, each `status="complete"`, `qty=1`.
...
```bash
docker logs pick-morph-camunda-spring-camunda-1 --since 10s 2>&1 | \
  grep -E "4feb_221|Updated ae_order|Enqueued ItemPickedEvent|Enqueued OrderUpdateEvent"
# Expected:
#   "Updated ae_order for pickInstructionId: 4feb_221, state: pick_transaction"
#   "Enqueued ItemPickedEvent | pickInstructionId: 4feb_221 | txId: 3000000001 | containerStatus: complete | danglingArea: bot"
#   "Enqueued ItemPickedEvent | pickInstructionId: 4feb_221 | txId: 3000000002 | containerStatus: complete | danglingArea: bot"
#   "Enqueued OrderUpdateEvent | pickInstructionId: 4feb_221 | orderline: 0 | state: pick_transaction | sub_state: in_progress"
...
docker exec pick-morph-camunda-postgres-1 psql -U appuser -d app_db -c \
  "SELECT pick_instruction_id, transaction_id, status,
   ...
   FROM transaction_status WHERE transaction_id IN ('3000000001','3000000002');"
...
# Expected: two messages with danglingArea=bot, picked_qty=1 each (txId=3000000001 and txId=3000000002)
```

> **`gor.item_picked.events` IS published** — two events, both `status=complete` → `danglingArea="bot"`, `qty_picked=1` each.
...
docker exec pick-morph-camunda-kafka-1 /opt/kafka/bin/kafka-console-consumer.sh \
  --bootstrap-server localhost:9092 --topic gor.item_picked.events --from-beginning --timeout-ms 3000 2>/dev/null
# Expected: four messages total — two from step 3b (danglingArea=bot, txId=3000000001 and 3000000002); two from step 4c (danglingArea=undefined, txId=3000000001 and 3000000002)
